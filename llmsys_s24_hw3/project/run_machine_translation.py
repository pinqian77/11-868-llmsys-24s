...
def collate_batch(examples, src_key, tgt_key, tokenizer, model_max_length, backend):
    ...
    for example in examples:
        ...
        input_ids_example = token_ids_src + token_ids_tgt
        labels_example = None
        label_token_weights_example = None

        # Case 1: src + tgt <= model_max_length
        if len(input_ids_example) <= model_max_length:
            pad_len = model_max_length - len(input_ids_example)
            input_ids_example += [pad_token_id] * pad_len
            labels_example = input_ids_example[1:] + [pad_token_id]
            label_token_weights_example = [0] * len(token_ids_src) + [1] * len(token_ids_tgt) + [0] * pad_len
        # Case 2: src + tgt > model_max_length
        else:
            labels_example = input_ids_example[1:model_max_length + 1]
            input_ids_example = input_ids_example[:model_max_length]
            
            token_ids_src_len = min(len(token_ids_src), model_max_length)
            label_token_weights_example = [0] * token_ids_src_len + [1] * (model_max_length - token_ids_src_len)
            
        if len(label_token_weights_example) != model_max_length or len(input_ids_example) != model_max_length or len(labels_example) != model_max_length:
            print("[collate_batch] Error: Length not match!!!!!!!!!!!!!!!!!!!!!!!!!!")
            print(f"token_ids_src: {token_ids_src}, len: {len(token_ids_src)}")
            print(f"token_ids_tgt: {token_ids_tgt}, len: {len(token_ids_tgt)}")
            print(f"input_ids_example: {input_ids_example}, len: {len(input_ids_example)}")
            print(f"labels_example: {labels_example}, len: {len(labels_example)}")
            print(f"label_token_weights_example: {label_token_weights_example}, len: {len(label_token_weights_example)}")

        assert len(input_ids_example) == model_max_length
        assert len(labels_example) == model_max_length
        assert len(label_token_weights_example) == model_max_length

        token_ids.append(input_ids_example)
        labels.append(labels_example)
        tgt_token_mask.append(label_token_weights_example)
    ...
    token_ids_tensor = minitorch.tensor(token_ids, backend=backend)
    labels_tensor = minitorch.tensor(labels, backend=backend)
    label_token_weights_tensor = minitorch.tensor(tgt_token_mask, backend=backend)

    # print(f"token_ids.shape: {token_ids_tensor.shape}")
    # print(f"labels.shape: {labels_tensor.shape}")
    # print(f"label_token_weights.shape: {label_token_weights_tensor.shape}")

    return {
        'input_ids': token_ids_tensor,
        'labels': labels_tensor,
        'label_token_weights': label_token_weights_tensor
    }
    # END ASSIGN2_2
...
def generate(model,
    ...
    for example in tqdm.tqdm(examples, desc=f'Generating {desc}'):
        ...
        while len(token_ids) <= model_max_length:
            ...
            logits = model(idx=minitorch.tensor([token_ids], backend=backend)) # should be (1, len(token_ids), vocab_size)
            # print(f"[generate] logits.shape: {logits.shape}")

            gen_id = minitorch.nn.argmax(logits, dim=2)  # argmax return a tensor with 1 on highest cell in dim, 0 otherwise
            # print(f"[generate] gen_id.shape: {gen_id.shape}")

            # get the nonzeor index using numpy
            gen_id = gen_id.to_numpy()
            # print(f"[generate] gen_id.to_numpy(): {gen_id}")
            gen_id = gen_id.nonzero()
            # print(f"[generate] gen_id.nonzero(): {gen_id}")
            gen_id = gen_id[-1][-1]
            # print(f"[generate] gen_id.nonzero(): {gen_id}")
            # END ASSIGN2_2
...
def parse_args():
    ...
    parser.add_argument('--use-fused-kernel', type=str2bool, default=True)
