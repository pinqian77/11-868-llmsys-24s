...
class LayerNorm1d(Module):
    def __init__(self, dim: int, eps: float, backend: TensorBackend, use_fused_kernel: bool = False):
        ...
        if self.use_fused_kernel:
            self.gamma = Parameter(tensor_from_numpy(np.ones((dim,)), backend=backend, requires_grad=True))
            self.beta = Parameter(tensor_from_numpy(np.zeros((dim,)), backend=backend, requires_grad=True))
