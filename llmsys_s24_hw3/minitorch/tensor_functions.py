...
class PowerScalar(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        # COPY FROM ASSIGN2_1
        grad_a = scalar * a.f.pow_scalar_zip(a, scalar - 1) * grad_output
...
class Tanh(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        """
        # COPY FROM ASSIGN2_1
        tanh_out, = ctx.saved_values

        one = minitorch.tensor([1.0], requires_grad=False, backend=tanh_out.backend)
        grad_a = (one - tanh_out ** 2) * grad_output
        # grad_a = (- tanh_out ** 2 + 1) * grad_output
...
class Attn_Softmax(Function):
    ...
    def backward(ctx: Context, out_grad: Tensor) -> Tensor:
      #   BEGIN ASSIGN3_1 
        (soft_inp,) = ctx.saved_tensors
        inp_grad = soft_inp.f.attn_softmax_bw(out_grad, soft_inp)
        return inp_grad, 0.0
      #   END ASSIGN3_1
