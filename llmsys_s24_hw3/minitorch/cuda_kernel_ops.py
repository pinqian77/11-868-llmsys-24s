...
class CudaKernelOps(TensorOps):
    ...
    def layernorm_fw(inp: Tensor, gamma: Tensor, beta: Tensor):
      #   BEGIN ASSIGN3_2
        ...
        # Define argument types for the CUDA function, if needed
        # This part depends on how your CUDA kernel is implemented and may vary
        lib_layernorm.launch_layernorm.argtypes = [
            # Assuming the function signature is similar to the one provided at the beginning
            np.ctypeslib.ndpointer(dtype=datatype, ndim=1, flags='C_CONTIGUOUS'),
            np.ctypeslib.ndpointer(dtype=datatype, ndim=1, flags='C_CONTIGUOUS'),
            np.ctypeslib.ndpointer(dtype=datatype, ndim=1, flags='C_CONTIGUOUS'),
            np.ctypeslib.ndpointer(dtype=datatype, ndim=1, flags='C_CONTIGUOUS'),
        ...
        ln_res = tensor_from_numpy(np.zeros((batch_size, hidden_dim)), backend=inp.backend)
        ...
        if inp._tensor._storage is None:
            print("inp is None")
    ...
    def layernorm_bw(out_grad: Tensor, inp: Tensor, gamma: Tensor, beta: Tensor, var: Tensor, mean: Tensor):
      #   BEGIN ASSIGN3_2
        ...
        inp_grad = tensor_from_numpy(np.zeros((batch_size, hidden_dim)), backend=inp.backend)
