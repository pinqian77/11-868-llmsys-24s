...
__global__ void ker_attn_softmax(T *inp, const T *attn_mask, int from_len,
  ...
  if (attn_mask) {
    ...
    // Debug: Check for NaN after loading mval
    // if (threadIdx.x == 0) {
    //     for (int j = 0; j < ele_per_thread; j++) {
    //         if (isnan(mval[j])) printf("[Load mval] NaN found in mval[%d]\n", j);
    //     }
    // }
  }
  ...
  for (int token_id = blockIdx.x * token_per_reduce; token_id < from_len;
    ...
    // Debug: Initial value check
    // if (threadIdx.x == 0) {
    //   for (int i = 0; i < token_per_reduce; i++) {
    //       for (int j = 0; j < ele_per_thread; j++) {
    //           if (isnan(inp_val[i][j])) printf("[Initial Values] NaN found in inp_val[%d][%d]\n", i, j);
    //       }
    //   }
    // }
    ...
    if (threadIdx.x == 0) {
      for (int i = 0; i < token_per_reduce; i++) {
        s_max[i] = l_max[i];
        // if (isnan(s_max[i])) printf("[Block Reduce Max] NaN found in s_max[%d]\n", i);
      }
    }
    __syncthreads();
    ...
    for (int i = 0; i < token_per_reduce; i++) {
      ...
      // if (isnan(l_sum[i])) printf("[Compute Sum] NaN found in l_sum[%d]\n", i);
    }

    // Debug: After computing sum
    // __syncthreads(); // Ensure all threads have computed their local sum before reducing
    // if (isnan(l_sum[0])) printf("[Compute Sum] NaN found in l_sum\n");
    // END ASSIGN3_1
    // block reduce sum
    ...
    if (threadIdx.x == 0) {
      for (int i = 0; i < token_per_reduce; i++) {
        // if (isnan(l_sum[i])) printf("[Block Reduce Sum] NaN found in l_sum[%d]\n", i);
        s_sum[i] = __fdividef(1.0f, l_sum[i] + EPSILON);
        // if (isnan(s_sum[i])) printf("[Divide for Softmax] NaN found in s_sum[%d], l_sum: %f\n", i, l_sum[i]);
      }
    }
    __syncthreads();
   ...
   for (int i = 0; i < token_per_reduce && (token_id + i) < from_len; i++) {
      for (int j = 0; j < ele_per_thread; j++) {
        // if (isnan(val[i][j])) printf("[Final Result] NaN found in val[%d][%d]\n", i, j);
        // if (isnan(s_sum[i])) printf("[Final Result] NaN found in s_sum[%d]\n", i);
        inp_val[i][j] = (T)(val[i][j] * s_sum[i]);
        // if (isnan(inp_val[i][j])) printf("[Final Result] NaN found in inp_val[%d][%d]\n", i, j);
      }
...
void launch_attn_softmax_bw(float *out_grad,
  ...
  if (softmax_len <= 32)
    ker_attn_softmax_bw<float, 1>
        <<<grid_dim, block_dim, 0, stream>>>(d_out_grad, d_soft_inp, softmax_len);
