...
class Linear(minitorch.Module):
    ...
    def forward(self, x):
        ...
        # 4. Add self.bias
        return (x @ weights).view(batch, self.out_size) + bias
        # HINT: You can use the view function of minitorch.tensor for reshapes
...
class Network(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        # Average embeddings across sentence length
        x = embeddings.mean(dim=1)
        x = x.view(embeddings.shape[0], embeddings.shape[2])
        
        # First linear layer
        x = self.linear1(x)
        
        # ReLU and Dropout
        x = x.relu() # x = minitorch.tensor.relu(x)
        x = minitorch.dropout(x, rate=self.dropout_prob)
        
        # Second linear layer
        x = self.linear2(x)
        
        # Sigmoid and reshape
        x = x.sigmoid() # x = minitorch.tensor.sigmoid(x)
...
def binary_cross_entropy(output, target):
    ...
    """
    epsilon = minitorch.tensor([1e-7])
    ones = minitorch.tensor([1.0] * target.shape[0], backend=BACKEND, requires_grad=True)
    
    loss = - (target * (output + epsilon).log() + (ones - target) * (ones - output + epsilon).log())
    return loss.mean()
    ...
    def train(
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            for batch_num, example_num in enumerate(
                ...
                end_index = min(example_num + batch_size, n_training_samples)
                x_batch = X_train[example_num:end_index]
                y_batch = y_train[example_num:end_index]

                # Convert data to tensors, ensuring they are on the correct backend and have gradients enabled for inputs
                ...
                # Optimizer step
                optim.step()
                optim.zero_grad()  # Clear gradients after update
            ...
            if data_val is not None:
                ...
                # Convert to tensors without requiring gradients
                x = minitorch.tensor(X_val, requires_grad=False, backend=BACKEND)
                y = minitorch.tensor(y_val, requires_grad=False, backend=BACKEND)
