...
def logsumexp(input: Tensor, dim: int) -> Tensor:
    ...
    return (input - Max.apply(input, tensor([dim]))).exp().sum(dim=dim).log() + Max.apply(input, tensor([dim]))
    ### END YOUR SOLUTION
...
def one_hot(input: Tensor, num_classes: int) -> Tensor:
    ...
    one_hot_encoded = identity_matrix[input.to_numpy().astype(int)]
    # print(f"input.to_numpy().astype(int): {input.to_numpy().astype(int)}")
    # print(f"one_hot_encoded: {one_hot_encoded}")
    # print(f"one_hot_encoded.shape: {one_hot_encoded.shape}")
    # print(f"one_hot_encoded[9]: {one_hot_encoded[0][0][9]}")
    
    return tensor_from_numpy(one_hot_encoded, backend=input.backend).view(*input.shape, num_classes)
    ### END YOUR SOLUTION
...
def softmax_loss(logits: Tensor, target: Tensor) -> Tensor:
    ...
    # Calculate the one-hot encoding
    one_hot_target = one_hot(target, num_classes)
    # # Calculate the loss
    result = - (one_hot_target * logsoftmax(logits, dim=1)).sum(dim=1)
    ### END YOUR SOLUTION
